...
try {
  ...
  // fallback to empty config if site.json reading fails
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    root: process.cwd(),
    base: process.env.GITHUB_PAGES ? '/Pixel-Pet/' : './',
    build: {
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Pluto — Your desk pet | Pastel Terminal Desktop Companion"
